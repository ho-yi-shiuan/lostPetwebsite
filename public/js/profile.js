	...
	if(res.length > 0){
		for(i=0; i<res.length; i++){
			...
			if(res[i].post_type == "find"){
				time.appendChild(document.createTextNode("發現時間: "+res[i].lost_time));					
			}else{
				time.appendChild(document.createTextNode("走失時間: "+res[i].lost_time));		
			}
			...
			if(res[i].post_type == "lost" && res[i].lost_status == "finding"){
				...
				var button_value = document.createTextNode("關閉協尋");
...
function message_to_room(room_id){
	...
	console.log(data);
	let socket_promise = new Promise((resolve, reject) => {
		socket.emit('message_to_backend', data);
		resolve("send_message");
	})
	socket_promise.then(function(){
